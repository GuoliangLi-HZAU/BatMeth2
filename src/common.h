...
//printf("\nA22 %d %d %d %d %c %c\n", A1.Loc1, A2.Loc1, A1.QualityScore, A2.QualityScore, A1.swscore1 + A1.swscore2, A2.swscore1 + A2.swscore2);
				if(abs(A1.Score) > abs(A2.Score)/*A1.QualityScore < A2.QualityScore*/)return true; 
				else if(abs(A1.Score) == abs(A2.Score)){
					if( A1.QualityScore > A2.QualityScore) return true;
					else return false;
				}else return false;
			}
			if(A1.paired==1)
