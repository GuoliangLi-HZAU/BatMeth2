...
void Print_Sam(READ & RawR,std::priority_queue <Alignment,std::vector <Alignment>,Comp_Alignment> & Final_Alignments,unsigned char* Original_Text,char source,FILE* Single_File,READ & R,Hit_Info & H,int StringLength,int Quality_Score,Alignment A,int TClip_H,int TClip_T,char* TCIG)
{
	...
	//if(H.Mismatch <=MAX_MISMATCHES && RawR.Real_Len==R.Real_Len)  //&& R.Real_Len*0.02+1 >=ed
	//	fprintf(Single_File,"%c\t%s\t%c\t%u\t%d\t%d\t%s\n",source,H.Chr,H.Sign,H.Loc,/*H.Mismatch*/Top_Penalty,R.Real_Len,CIG);//Quality_Score,%d\t
	//printf("\n%d %d %d %d %d %d %d %d %s\n",RawR.Real_Len,R.Real_Len,Get_ED(CIG),H.Mismatch,Quality_Score,MAX_MISMATCHES, (int)(3*((double)R.Real_Len/100)),H.Loc,CIG);
	//if(H.Mismatch <=MAX_MISMATCHES && RawR.Real_Len==R.Real_Len && Get_ED(CIG) < (int)(2*((double)R.Real_Len/100)) ) //&& R.Real_Len*0.02+1 >=ed
	{
...
int Calc_MapQ(Hit_Info & H,Alignment & A,int Clip_Count)
{
	...
	else if(H.Status==UNIQUEHIT)
	{
		...
		if(Quality_Score>1 && Quality_Score<55) 
			Quality_Score+=5;
		//if(!STACK_LOWQ && Quality_Score==1)
			//Quality_Score=0;
	...
	else 
	{
		...
		if(A.Score<A.Sub_Opt_Score-SCOREGAP)//Q_GAP/3 )
		{
			...
			{
				if(Quality_Score<55) Quality_Score+=5;
				return Quality_Score;
			}
			/*if(!STACK_LOWQ && Quality_Score==1)
				Quality_Score=0;
