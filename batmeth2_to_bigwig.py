...
def processFile( allCFileStr, chrmFileStr, label, outID, baseMod, keepTmp, isSort, printStrand):
	...
	OutCHfileStr = bedGraphStr + '.ch.bedGraph'

	readAllC( allCFileStr, bedGraphAr, OutCfileStr ,OutCHfileStr ,printStrand)
	...
	processBedGraph(OutCHfileStr, chrmFileStr)
	# remove temporary
	if keepTmp == False:
		...
		os.remove(OutCHfileStr)
	print( 'BigWig finished for {:s}.bw.*'.format( outID ) )

def readAllC( allCFileStr, outFileAr, OutCfileStr, OutCHfileStr, printStrand):
