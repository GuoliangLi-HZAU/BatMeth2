  ...
  while (in >> chrom >> pos >> strand >> seq >> meth >> coverage) { //changed
    ...
    if (chrom.empty() || strand.empty() || seq.empty() ||
      ...
      oss << chrom << "\t" << pos << "\t" << strand << "\t"
          << seq << "\t" << meth << "\t" << coverage << "\n";
      throw DNAmethException("Invalid input line:" + oss.str());
    }

    // order check
    if (prev_chrom > chrom || (prev_chrom == chrom && prev_pos > pos)) {
      throw DNAmethException("CpGs not sorted in file \"" + cpgs_file + "\"");
    }
    prev_chrom = chrom;
    prev_pos = pos;
...
methpipe::is_methpipe_file_single(const string &file) {
  ...
  if (!in)
    throw DNAmethException("could not open file: " + file);

  string line;
  if (!std::getline(in, line))
    throw DNAmethException("could not read file: " + file);
  line = skip_header(in);
                        ...
                        double &meth, size_t &coverage)
{
  ...
  if (iss >> chrom >> pos >> dummy >> name >> meth >> strand) {
    ...
    throw DNAmethException("Invalid input line:" + line);
  return in;
}
