...
bool Find_Paired(int ReadLen1, int ReadLen2,int & paired_score,bool & Unique,std::priority_queue <Alignment,std::vector <Alignment>,Comp_Alignment> & A,std::priority_queue <Alignment,std::vector <Alignment>,Comp_Alignment> & B,std::map<unsigned,Alignment> & D,std::map<unsigned,Alignment> & D_P,int Extra_Bit=0,int SW_Compare=0);
...
//printf("\n%s\n",R.Description);
		...
		Mapping_Batmis(align_mis,Paired_Score,'1','3',R_CT,M_GA,B_batmeth,MF_CT,MC_CT,MF_CT2,MC_CT2,mF_CT,mC_CT,L_batmeth,fwfmiCT,revfmiCT,R,M,Align_Hits_CT,Align_Hits_CT_P,O_CT,O_RevCT,Original_Text_CT,Single_File,Alignments_Reslut,MFLH_CT,MCLH_CT,MFLT_CT,MCLT_CT,MFH_CT,MCH_CT,MFT_CT,MCT_CT, L,L_Main,L_Half,L_Third,Actual_Tag,Mishit_File,Pairs,Conversion_Factor,Entries_CT,RQHALF_CT,RQ_CT);
		Mapping_Batmis(align_mis,Paired_Score,'4','2',R_CT,M_GA,B_batmeth,MF_GA,MC_GA,MF_GA2,MC_GA2,mF_GA,mC_GA,L_batmeth,fwfmiGA,revfmiGA,R,M,Align_Hits_CT,Align_Hits_CT_P,O_RevGA,O_GA,Original_Text_GA,Single_File,Alignments_Reslut,MFLH_GA,MCLH_GA,MFLT_GA,MCLT_GA,MFH_GA,MCH_GA,MFT_GA,MCT_GA,L,L_Main,L_Half,L_Third,Actual_Tag,Mishit_File,Pairs,Conversion_Factor,Entries_GA,RQHALF_GA,RQ_GA);
		if(!Alignments_Reslut.empty() && !ESTIMATE )
		{    
			Alignment_Pair pai=Alignments_Reslut.top();
			if(pai.paired==1 && (pai.Mismatch==0 && pai.Indel==0) ) //pai.Mismatch1<=2 && pai.Mismatch2<=2)
			{
				...
				fprintf(Single_File,"%s\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%s\t%s\tNM:i:%d\tAS:i:%d\tSA:i:%d\tSW:i:%d\tSS:i:%d\n", pai.Description1+1, pai.Flag1, pai.chrom1, pai.Loc1, (pai.Q1+pai.Q2)/2, pai.Cigar1.c_str(), (pai.Same_Chrom? "=":pai.chrom2), pai.Loc2, pai.Insert_Size, seq1.c_str(), qual1.c_str(), pai.Mismatch1, pai.Score, pai2.Score, pai.swscore1, pai2.swscore1);
				fprintf(Single_File,"%s\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%s\t%s\tNM:i:%d\tAS:i:%d\tSA:i:%d\tSW:i:%d\tSS:i:%d\n", pai.Description2+1, pai.Flag2, pai.chrom2, pai.Loc2, (pai.Q1+pai.Q2)/2, pai.Cigar2.c_str(), (pai.Same_Chrom? "=":pai.chrom1), pai.Loc1, -pai.Insert_Size, seq2.c_str(), qual2.c_str(), pai.Mismatch2, pai.Score, pai2.Score, pai.swscore2, pai2.swscore2);
       ...
       align_mis=1;
		...
		{    
                        Alignment_Pair pai=Alignments_Reslut.top();
			if(pai.paired==1 && ( (pai.Mismatch+pai.Clip<=1 && pai.Indel==0 ) || (pai.Mismatch==0 && pai.Indel<=1 && pai.Clip==0) ) )
			{
                	        Remove_Dup_Final(Alignments_Reslut);
        	                Alignment_Pair pai=Alignments_Reslut.top();Alignments_Reslut.pop();
	                        Alignment_Pair pai2; pai2.Score = 0; pai2.swscore1 = 0; pai2.swscore2 = 0;
		...
		{    
                        Alignment_Pair pai=Alignments_Reslut.top();
			if(pai.paired==1 && ( (pai.Mismatch+pai.Clip<=3 && pai.Indel==0) || (pai.Mismatch+pai.Clip<=1 && pai.Indel<=1) ) )
			{
                        	Remove_Dup_Final(Alignments_Reslut);
                	        Alignment_Pair pai=Alignments_Reslut.top();Alignments_Reslut.pop();
        	                Alignment_Pair pai2; pai2.Score = 0; pai2.swscore1 = 0; pai2.swscore2 = 0;
...
//			{//rm
			...
			if(pai.paired)
			{
				...
				if(pai.paired==1)
				{
                                	fprintf(Single_File,"%s\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%s\t%s\tNM:i:%d\tAS:i:%d\tSA:i:%d\tSW:i:%d\tSS:i:%d\n", pai.Description1+1, pai.Flag1, pai.chrom1, pai.Loc1, pai.Same_Chrom? (pai.Q1+pai.Q2)/2 : pai.Q1, pai.Cigar1.c_str(), (pai.Same_Chrom? "=":pai.chrom2), pai.Loc2, pai.Insert_Size, seq1.c_str(), qual1.c_str(), pai.Mismatch1, pai.Score, pai2.Score, pai.swscore1, pai2.swscore1);
                                	fprintf(Single_File,"%s\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%s\t%s\tNM:i:%d\tAS:i:%d\tSA:i:%d\tSW:i:%d\tSS:i:%d\n", pai.Description2+1, pai.Flag2, pai.chrom2, pai.Loc2, pai.Same_Chrom? (pai.Q1+pai.Q2)/2 : pai.Q2, pai.Cigar2.c_str(), (pai.Same_Chrom? "=":pai.chrom1), pai.Loc1, -pai.Insert_Size, seq2.c_str(), qual2.c_str(), pai.Mismatch2, pai.Score, pai2.Score, pai.swscore2, pai2.swscore2);
				}else {
					fprintf(Single_File,"%s\t%d\t%s\t%d\t%d\t%s\t*\t0\t0\t%s\t%s\tNM:i:%d\tAS:i:%d\tSA:i:%d\tSW:i:%d\tSS:i:%d\n", pai.Description1+1, pai.Flag1, pai.chrom1, pai.Loc1, pai.Same_Chrom? (pai.Q1+pai.Q2)/2 : pai.Q1, pai.Cigar1.c_str(), seq1.c_str(), qual1.c_str(), pai.Mismatch1, pai.Score, pai2.Score, pai.swscore1, pai2.swscore1);
					fprintf(Single_File,"%s\t%d\t%s\t%d\t%d\t%s\t*\t0\t0\t%s\t%s\tNM:i:%d\tAS:i:%d\tSA:i:%d\tSW:i:%d\tSS:i:%d\n", pai.Description2+1, pai.Flag2, pai.chrom2, pai.Loc2, pai.Same_Chrom? (pai.Q1+pai.Q2)/2 : pai.Q2, pai.Cigar2.c_str(), seq2.c_str(), qual2.c_str(), pai.Mismatch2, pai.Score, pai2.Score, pai.swscore2, pai2.swscore2);
				}
			}else 
			{
...
void Attached_mq(Alignment_Pair & pai, Alignment_Pair & pai2, int Real_Len1, int Real_Len2) {
	...
	if(pai2.swscore >= pai.swscore || pai2.Score <= pai.Score) pai.QualityScore = pai.QualityScore<pai2.QualityScore?pai.QualityScore:pai2.QualityScore;
        if( (strcmp(pai.chrom1, pai2.chrom1) != 0 || dist(pai2.Loc1, pai.Loc1)>100) && dist(pai.swscore1, pai2.swscore1)<=5) pai.Q1 = dist(pai.swscore1, pai2.swscore1);
        if(pai.paired !=0 && (strcmp(pai.chrom2, pai2.chrom2) != 0 || dist(pai2.Loc2, pai.Loc2)>100) && dist(pai.swscore2, pai2.swscore2)<=5) pai.Q2 = dist(pai.swscore1, pai2.swscore1);
        ...
        if( (strcmp(pai.chrom1, pai2.chrom1) != 0 || dist(pai2.Loc1, pai.Loc1)>100) && dist(pai.swscore1, pai2.swscore1)<=30 && pai.Q1 >= 30 ) pai.Q1 = Cal_mapQ(pai2.swscore1, pai.swscore1, pai.Cigar1.c_str(), pai.Mismatch1, Real_Len1);
	if(pai.paired !=0 && (strcmp(pai.chrom2, pai2.chrom2) != 0 || dist(pai2.Loc2, pai.Loc2)>100) && dist(pai.swscore2, pai2.swscore2)<=30 && pai.Q2 >= 30 ) pai.Q2 = Cal_mapQ(pai2.swscore2, pai.swscore2, pai.Cigar2.c_str(), pai.Mismatch2, Real_Len2);
...
//printf("\nccgiar %s\n",Alignments_P.top().Cigar);
	...
	if( realign && (Align_Difference(Alignments,A1.Loc) || Align_Difference(Alignments_P,A1_P.Loc)) )//check if actual best alignment differes from the initial..
	{
		...
		if(!Find_Paired(RawR.Real_Len, RawM.Real_Len, paired_score,unique,Alignments,Alignments_P,D,D_P))
		{
...
{	
	...
	bool unique=true;int paired_score=INT_MAX;
	bool FP=Find_Paired(RawR.Real_Len, RawM.Real_Len, paired_score,unique,Alignments,Alignments_P,D,D_P);
	...
	if(!Alignments.empty() && !Alignments_P.empty() && Find_Paired(RawR.Real_Len, RawM.Real_Len, paired_score,unique,Alignments,Alignments_P,D,D_P,Read_Length))
	{
...
const int MAX_PROPER_PAIRS=3000;//moxian //30000
bool Find_Paired(int ReadLen1, int ReadLen2, int & paired_score,bool & Unique,std::priority_queue <Alignment,std::vector <Alignment>,Comp_Alignment> & A,std::priority_queue <Alignment,std::vector <Alignment>,Comp_Alignment> & B,std::map<unsigned,Alignment> & D,std::map<unsigned,Alignment> & D_P,int Extra_Bit,int SW_Compare)
{
...
//printf("\nGGGH %d %d %d %s %d %d %d %u %u\n", abs(Nearest_Pair->first-I->first), (I->second).Score, (Nearest_Pair->second).Score, (Nearest_Pair->second).Cigar, (Nearest_Pair->second).Mismatch, Correct_Orientation(I->second,Nearest_Pair->second,Extra_Bit), Correct_Ref(I->second,Nearest_Pair->second), Nearest_Pair->first, I->first);
		while(Nearest_Pair!=D_P.end() && (abs(Nearest_Pair->first-I->first) < INSERTSIZE+3*STD+Extra_Bit))
		{
			...
			if (Correct_Orientation(I->second,Nearest_Pair->second,Extra_Bit))
			{
				...
				{
					if(Paired_Score < (Head.Score+Tail.Score)+30)
					{
						Unique=false;
					}
					else
...
//printf("\nHead %d %d %d %d %u %u %d %d %d\n",Head.Score,Head.Mismatch,Tail.Score,Tail.Mismatch, Head.Loc, Tail.Loc, Sub_Opt_Score ,Pairings_Index, Unique);
...
//printf("\n-=-=-= %s %ld %d %d || %d %d\n",Alignments_tmp.top().Cigar,Alignments_tmp.top().Loc,realign,RTemp.Real_Len,mismatch,Indel);
	...
	bool unique=true;int paired_score=INT_MAX;
	Find_Paired(RawR.Real_Len, RawM.Real_Len, paired_score,unique,Alignments,Alignments_P,D,D_P,Read_Length);
...
void Print_Pair(std::priority_queue <Alignment_Pair,std::vector <Alignment_Pair>,Comp_Align_Pair> & Alignments_Reslut,Hit_Info &  HitsH,Hit_Info &  HitsT,char Sign1,char Sign2,READ & RawR,READ & RawM,unsigned char* Original_Text,char source1,char source2,FILE* Single_File,Final_Hit & H,Final_Hit & T,READ & R1, READ & R2,MEMX & MF,MEMX & MC,BWT* fwfmi,BWT* revfmi)
{
	...
	if (H.Loc != INT_MAX && T.Loc != INT_MAX)
	{
		...
		//if( H.Mismatch <=BP.MAX_MISMATCHES && T.Mismatch <=BP.MAX_MISMATCHES) //((R1.Real_Len*0.02+1 >= ed1) || (R2.Real_Len*0.02+1 >= ed2) ) && //MISMATCH
		{
			...
			if(Same_Chrome && abs(Insert_Size)<= INSERTSIZE+3*STD ) Pai.paired=1;//true;
