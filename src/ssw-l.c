		...
		for (i = 0; i < segLen; i ++) {
			...
			for (segNum = 0; LIKELY(segNum < 8) ; segNum ++) {
				*t++ = j>= readLen ? 0 : mat[nt * n + read_num[j]];
				//printf("\n== %u; %d %d %d %d %d %d %d\n",*t,nt * n + read_num[j],nt,n,read_num[j],j,readLen,mat[nt * n + read_num[j]]);
				j += segLen;
			}
		}
	}
...
void init_SSW_BS_c2t(int8_t* mata) {
	match = 2;
	mismatch = 2;
	gap_open = 3;//11;//3;
	gap_extension = 1;
	...
	//mata[8]=match;//Ref:C read:T
	for (m = 0; LIKELY(m < 5); ++m) mata[k++] = 0; //ambiguous base
}

void init_SSW_BS_g2a(int8_t* mata) {
	match = 2;
	mismatch = 2;
	gap_open = 3;//11;//3;
	gap_extension = 1;
	...
	int32_t l, m, k;
	//mata = (int8_t*)calloc(25, sizeof(int8_t));
	for (l = k = 0; LIKELY(l < 4); ++l) {//0 1 2 3
		for (m = 0; LIKELY(m < 4); ++m) mata[k++] = l == m ? match : -mismatch;	/* weight_match : -weight_mismatch */
		mata[k++] = 0; // ambiguous base
	}
	//mata[10]=match; //ref:G read:A
	for (m = 0; LIKELY(m < 5); ++m) mata[k++] = 0; //ambiguous base
	
}

void init_SSW(int8_t* mata) {
	match = 2;
	mismatch = 2;
	gap_open = 3;//11;//3;
	gap_extension = 1;
