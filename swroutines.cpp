...
void ssw_cigar_processQ(char Sign,char* rawRead,char* rawRef,s_align* a,Cigar_Info & C,char* Ref,int Ref_Off,char* Pattern,int Pat_Off,int StringLength,const char* Qual,char* Cigar,int Clip_H,int Clip_T,bool Hard_Penalty) { //print the cigar out //,READ & R
	...
	int c = 0,Tot_Length=0;char *Cigar_Ptr=new char[MAX_CIG];//(char *)malloc(sizeof(char)*50); //char* Cigar_Ptr=Cigar;
	char *Cigar_Temp=new char[MAX_CIG];
...
//if(!ESTIMATE) printf("\n%d %s %s\n",Clip_H,Pattern,Qual);
	...
	if(Cigar && Clip_H) 
	{
		sprintf(Cigar_Temp,"%dS", Clip_H-lenM); 
		strcat(Cigar_Ptr,Cigar_Temp);
	}
/////////////////////////////////////////////////////////////////////
	...
	for (c = 0; c < a->cigarLen; ++c) {
		...
		if(Cigar && c==1) 
		{
			sprintf(Cigar_Temp,"%d", length+lenM);
			strcat(Cigar_Ptr,Cigar_Temp);
		}
		else if(Cigar) 
		{
			sprintf(Cigar_Temp,"%d", length);
			strcat(Cigar_Ptr,Cigar_Temp);
		}
	printf("\nletter %d %d\n",letter,length);
//	printf("\nlen %d %d %s",length,lenM,Cigar);
		if (letter == 0) 
		{
			Tot_Length+=length;
			Tot_Ref_Length+=length;
			if(Cigar) strcat(Cigar_Ptr,"M");
			for(int i=0;i<length;i++)
			{
				if(*Ref>4 || *Ref<0 || *Pattern>=4 || *Pattern<0)
				{
		...
		else if (letter == 1)
		{
			Tot_Length+=length;
			C.Indel_Count++;
			if(Cigar) strcat(Cigar_Ptr,"I");//*Cigar_Ptr='I';Cigar_Ptr++;
			C.I+=length;
		...
		else 
		{
			...
			Score+=(gap_open+length*gap_extension);
			BQScore-= BOPEN + length*BEXT;
			if(Cigar) strcat(Cigar_Ptr,"D");//*Cigar_Ptr='D';Cigar_Ptr++;
			C.D+=length;
	...
	if(Cigar)
	{
		...
		if(Clip_T) 
		{
			sprintf(Cigar_Temp,"%dS", Clip_T); 
			strcat(Cigar_Ptr,Cigar_Temp);
		}
		//*Cigar_Ptr=0;
		printf("\n%s\n",Cigar_Ptr);
		strcpy(Cigar,Cigar_Ptr);
		if(Cig_Err) *Cigar=0;
	}
