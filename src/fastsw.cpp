...
void Mismatch_Scan_With_ScoreX(char Sign,int ReadSpan,READ & RawR,char* rawRef,char source,char* Ref,char* Read,int Read_Len,int Max_Mis,int Current_Mis,Alignment & A)
{
	READ RawRs;
	if(Sign=='-')
	{
		int i;int Lens=RawR.Real_Len;
		for (i=0;i<=Lens-1;i++){RawRs.Tag_Copy[Lens-1-i]=Char_To_R[RawR.Tag_Copy[i]];};
	}
	else
	{
		RawRs=RawR;
	}
	char* rawRead =RawRs.Tag_Copy;//momo
	rawRead+=ReadSpan;
	int Mis=0;
	int SW_Score=0;
	...
	{
		if(*(Ref+j)!=*(Read+j) || (*(Ref+j)==*(rawRef+j) && *(rawRef+j)==3 && (*(rawRead+j)=='c' || *(rawRead+j)=='C' ) ) ||  (*(Ref+j)==*(rawRef+j) && *(rawRef+j)==0 && (*(rawRead+j)=='g' ||*(rawRead+j)=='G' ) ) ) 
		{
			Mis++;
			SW_Score-=mismatch;
			Score+=Mis_FA_Score;
...
int Mismatch_Scan(char Sign,int ReadSpan,char* rawRef,char* rawRead,char source,char* Ref,char* Read,int Read_Len,int Max_Mis,int Current_Mis)
{
	int Mis=0;
	int Score=0;
	for(int j=0;j<Read_Len && Mis<=Max_Mis;j++)
...
int MatchX(char Sign,int ReadSpan,char* rawRead,char* rawRef,char source,char* Ref,int Ref_Len,char* Read,int Read_Len,int Gap,int Max_Mis,Match_Info Pos[],int Current_Mis)
{
	...
	for(int i=0;(i<=Gap);i++)
	{
		...
		{
			if(*(Ref+j)!=*(Read+i+j) || (*(Ref+j)==*(rawRef+j) && *(rawRef+j)=='T' && (*(rawRead+i+j)=='c' || *(rawRead+i+j)=='C' ) ) ||  (*(Ref+j)==*(rawRef+j) && *(rawRef+j)=='A' && (*(rawRead+i+j)=='g' ||*(rawRead+i+j)=='G' ) ) ) 
			{Mis++;
			}
		}
...
int Match(char Sign,int ReadSpan,char* rawRef,char* rawRead,char source,char* Ref,int Ref_Len,char* Read,int Read_Len,int Gap,int Max_Mis,Match_Info Pos[],int Current_Mis)
{
...
Gap_Info Get_Max(int Sspan,int Lspan,char Sign,int ReadSpan,char* rawRead,char* rawRef,char source,char* S_Start,char* S_End,char* L_Start,char* L_End,Gap_Info *G_Array,int Mismatch_Limit)
{
...
Gap_Info Maximize_Ins(int Sspan,int Lspan,char Sign,int ReadSpan,char* rawRead,char* rawRef,char source,char* Read,char* Ref,char* Ref_End,char* Read_Init,Match_Info Pos[],int Delete,int Offset,int Mismatch_Limit,int Mis_In_Left,int Tot_Read_Len)
{
	int Pos_Ptr=0;
	...
	while(Pos[Pos_Ptr].Loc!=INT_MAX)
	{
		...
		Get_Max(Sspan+Loc-1,Lspan,Sign,ReadSpan,rawRead,rawRef,source,Read,Read_Init+Loc-1,Ref,Ref_End,G,Mismatch_Limit);
		int j=0;
...
Gap_Info Maximize_Del(int Sspan,int Lspan,char Sign,int ReadSpan,char* rawRead,char* rawRef,char source,char* Read,char* Ref,char* Ref_End,char* Read_Init,Match_Info Pos[],int Delete,int Offset,int Mismatch_Limit,int Mis_In_Left,int Tot_Read_Len)
{
	int Pos_Ptr=0;
	...
	while(Pos[Pos_Ptr].Loc!=INT_MAX)
	{
		...
		Get_Max(Sspan,Lspan+Loc-1,Sign,ReadSpan,rawRead,rawRef,source,Read,Read_Init,Ref,Ref_End+Loc-1,G,Mismatch_Limit);//momo
		int j=0;
		while(G[j].Start!=INT_MAX)
		{
...
int Insert_In_First_Half(char Sign,int ReadSpan,char* rawRef,char* rawRead,char source,char* D,char* S,int Delete,int Insert,int Max_Mis,int Offset,int Mis_In_Left,int Tot_Read_Len,Gap_Info & Gap_Alignment)
{
	int LS=strlen(S);
	int Backtrack=LS/2;
	Gap_Info G;
	...
	int S1=Match(Sign,LS/2+ReadSpan,rawRef+LS/2,rawRead+Backtrack,source,D+LS/2,strlen(D),S+Backtrack,(LS-Backtrack)/*LS/2+Delete)-Length*/,Gap,Max_Mis,Pos,Mis_In_Left);//Indel in first third
	assert(Max_Mis-Mis_In_Left>=0);
	if(S1)
	{
		//return Maximize(S,D,D+LS/2-1,S+Backtrack,Pos,LS/2,Delete,Offset);
		G=Maximize_Ins(Backtrack,LS/2-1,Sign,ReadSpan,rawRead,rawRef,source,S,D,D+LS/2-1,S+Backtrack,Pos,Delete,Offset,Max_Mis-Mis_In_Left,Mis_In_Left,Tot_Read_Len);
	...
	if(Insert>=LS/2)
	{
		...
		{
			G=Maximize_Ins(LS,i+1,Sign,ReadSpan,rawRead,rawRef,source,S,D,D+i+1,S+LS,Pos,0,Offset,Max_Mis-Mis_In_Left,Mis_In_Left,Tot_Read_Len);
			if(G.Score>Gap_Alignment.Score)
			{
				Gap_Alignment=G;
			}
...
int Delete_In_First_Half(char Sign,int ReadSpan,char* D_raw,char* S_raw,char source,char* D,char* S,int Delete,int Insert,int Max_Mis,int Offset,int Mis_In_Left,int Tot_Read_Len,Gap_Info & Gap_Alignment)
{
	int LS=strlen(S);
	int Backtrack=LS/2;
	...
	int S1=MatchX(Sign,LS/2+ReadSpan,S_raw+LS/2,D_raw+LS/2,source,D/*S*/+LS/2,strlen(S),S/*D*/+LS/2,(LS-LS/2)/*LS/2+Delete)-Length*/,Gap,Max_Mis,Pos,Mis_In_Left);//Indel in first third
	assert(Max_Mis-Mis_In_Left>=0);
	if(S1)
	{
		//return Maximize(S,D,D+LS/2-1,S+Backtrack,Pos,LS/2,Delete,Offset);
		Gap_Info G=Maximize_Del(LS/2-1,LS/2,Sign,ReadSpan,S_raw,D_raw,source,S,D,D+LS/2,S+LS/2-1,Pos,Delete,Offset,Max_Mis-Mis_In_Left,Mis_In_Left,Tot_Read_Len);
...
//	char *D_raw=rawRef+OFF,*S_raw=rawRead+OFF;
//	int Half_Length=Tot_Read_Len-OFF;

        Alignment A;
        A.Loc=0;A.Mismatch=0;A.SW_Score=0;A.Mismatch=0;A.swscore=0;
	A.Indel=0;A.BQScore=A.Score=A.QScore=A.Clip_H=A.Clip_T=0;

	Mismatch_Scan_With_Score(Sign,0,rawRead,source,Ref,Read,Quality,Tot_Read_Len,100,0,A,rawRef);
	//Mismatch_Scan_With_Score(Sign,OFF,S_raw,source,D,S,Q,Half_Length,100,0,A,D_raw);
	A.Score=-A.Score;
	sprintf(A.Cigar,"%dM",Tot_Read_Len);
//printf("\nScore_Fast %d %s %d %d %ld %d %c",A.Score,A.Cigar,A.BQScore,A.SW_Score,A.Loc,A.Mismatch,Sign);
...
Alignment Fast_SW(char*rawRead,char source,char *Ref,char *Read,char *Quality,int OFF,char Sign,char* rawRef,READ & RawR)
{
	...
	int TScore=Mismatch_Scan(Sign,OFF,D_raw,S_raw,source,D,S,Half_Length,100,0);
	if(TScore >0)
	{
	...
	do
	{
		...
		Max_Score=Delete_In_First_Half(Sign,OFF,D_raw,S_raw,source,D,S,DELETE,INSERT,Max_Mis,Offset,Mis_In_LH,Half_Length,Gap_Alignment);
		Read_Len=strlen(S);
		...
		D_raw=D_raw+Read_Len/2;S_raw=S_raw+Read_Len/2;
		Offset+=Read_Len/2;
	}
	...
	D_raw=rawRef+OFF,S_raw=rawRead+OFF;
	Offset=0;
	Mis_In_LH=0;
	Score_In_LH=0;
	do
	{
		Max_Score=Insert_In_First_Half(Sign,OFF,D_raw,S_raw,source,D,S,DELETE,INSERT,Max_Mis,Offset,Mis_In_LH,Half_Length,Gap_Alignment);
		Read_Len=strlen(S);
		Mis_In_LH+=Ham(D,S,Read_Len/2);
		if(Mis_In_LH>Max_Mis) break;
		//Score_In_LH=Mis_In_LH*MISMATCH+(Tot_Read_Len_LH-Max_Mis)*MATCH;
	...
	{
		A.SW_Score=0;A.Mismatch=0;A.swscore = 0;
		...
		if(Gap_Alignment.B)//Indel present
		{
			...
			Mismatch_Scan_With_Score(Sign,0,rawRead,source,Ref,Read,Quality,Skip,100,0,A,rawRef);//left half..

			if(Gap_Alignment.Indel_Flag=='I')
			{
				Mismatch_Scan_With_Score(Sign,Skip+Gap_Alignment.B,rawRead+Skip+Gap_Alignment.B,source,Ref+Skip,Read+Skip+Gap_Alignment.B,Quality+Skip+Gap_Alignment.B,Tot_Read_Len-Skip-Gap_Alignment.B,100,0,A,rawRef+Skip);
				A.Loc+=Gap_Alignment.B;
			}
			else
			{
				assert(Gap_Alignment.Indel_Flag=='D');
				Mismatch_Scan_With_Score(Sign,Skip,rawRead+Skip,source,Ref+Skip+Gap_Alignment.B,Read+Skip,Quality+Skip,Tot_Read_Len-Skip,100,0,A,rawRef+Skip+Gap_Alignment.B);
				A.Loc-=Gap_Alignment.B;
			}
			A.BQScore-=(BOPEN+BEXT*(Gap_Alignment.B));
			A.SW_Score-=(gap_open+Gap_Alignment.B*gap_extension);
			A.swscore=(gap_open+Gap_Alignment.B*gap_extension);
	...
	else
	{
		...
		A.SW_Score=0;
		A.swscore=0;
	}//printf("\nScore %d %s %d %d %ld",A.Score,A.Cigar,A.BQScore,A.SW_Score,A.Loc);
	return A;
}

Alignment Fast_SWX(char Sign,READ & RawR,char* rawRef,char source,char *Ref,char *Read,int OFF)
{
/*
	if(OFF==INT_MAX) OFF=SEEDSIZE;
	int Tot_Read_Len_Org=strlen(Read);
	...
	int Half_Length=Tot_Read_Len-OFF;
	Gap_Info Gap_Alignment;Gap_Alignment.Score= -1;
	int TScore=Mismatch_Scan(Sign,OFF,RawR,rawRef+OFF,source,D,S,Half_Length,100,0);
	if(TScore >0)
	{
		Gap_Alignment.Score=TScore;
		Gap_Alignment.A=Tot_Read_Len_Org;Gap_Alignment.B=Gap_Alignment.C=0;
	...
	{
		//Max_Score=Insert_In_First_Half(D,S,Delete,Insert,Max_Mis,Offset,Mis_In_LH,Tot_Read_Len);
		Max_Score=Delete_In_First_Half(Sign,OFF,RawR,rawRef+OFF,source,D,S,DELETE,INSERT,Max_Mis,Offset,Mis_In_LH,Half_Length,Gap_Alignment);
		Read_Len=strlen(S);
		Mis_In_LH+=Ham(D,S,Read_Len/2);
		if(Mis_In_LH>Max_Mis) break;
		//Score_In_LH=Mis_In_LH*MISMATCH+(Tot_Read_Len_LH-Max_Mis)*MATCH;
	...
	do
	{
		Max_Score=Insert_In_First_Half(Sign,OFF,RawR,rawRef+OFF,source,D,S,DELETE,INSERT,Max_Mis,Offset,Mis_In_LH,Half_Length,Gap_Alignment);
		Read_Len=strlen(S);
		Mis_In_LH+=Ham(D,S,Read_Len/2);
		if(Mis_In_LH>Max_Mis) break;
		//Score_In_LH=Mis_In_LH*MISMATCH+(Tot_Read_Len_LH-Max_Mis)*MATCH;
	...
	if(Gap_Alignment.Score>0 && Gap_Alignment.Score!=INT_MAX)
	{
		...
		{
			int Skip=Tot_Read_Len-Half_Length+Gap_Alignment.A;//skip from begin..
			Mismatch_Scan_With_ScoreX(Sign,0,RawR,rawRef,source,Ref,Read,Skip,100,0,A);//left half..
			if(Gap_Alignment.Indel_Flag=='I')
			{
				Mismatch_Scan_With_ScoreX(Sign,Skip+Gap_Alignment.B,RawR,rawRef+Skip,source,Ref+Skip,Read+Skip+Gap_Alignment.B,Tot_Read_Len-Skip-Gap_Alignment.B,100,0,A);
			}
			else
			{
				assert(Gap_Alignment.Indel_Flag=='D');
				//Mismatch_Scan_With_Score(Ref+Skip+Gap_Alignment.B,Read+Skip,Quality,Tot_Read_Len-Skip+Gap_Alignment.B,100,0,A);
				Mismatch_Scan_With_ScoreX(Sign,Skip,RawR,rawRef+Skip+Gap_Alignment.B,source,Ref+Skip+Gap_Alignment.B,Read+Skip,Tot_Read_Len-Skip,100,0,A);
			}
			A.BQScore-=(BOPEN+BEXT*(Gap_Alignment.B));
			A.SW_Score-=(gap_open+Gap_Alignment.B*gap_extension);
			A.Score+=(gap_openP+Gap_Alignment.B*gap_extensionP);
		...
		{
			Mismatch_Scan_With_ScoreX(Sign,0,RawR,rawRef,source,Ref,Read,Tot_Read_Len,100,0,A);
			sprintf(A.Cigar,"%dM",Gap_Alignment.A);
		}
		A.Score= -A.Score;
	}
