...
int sam_read1(tamFile fp, bam_header_t *header, bam1_t *b)
{
	...
	{ // name
                strcpy(str->s, str->s+1); //momo remove 'f' and 'r' in chrom start
	...
	{ // mtid, mpos, isize
		ret = ks_getuntil(ks, KS_SEP_TAB, str, &dret); z += str->l + 1;
                if(strcmp(str->s, "=") ==0 ){
                    ...
                    strcpy(str->s, str->s+1);
                    bam_get_tid(header, str->s);
                } // momo for remove 'f' and 'r' for matechrom
