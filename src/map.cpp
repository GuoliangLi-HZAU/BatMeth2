...
Zero:
        really_free(M.Left_Mishits);
        really_free(M.Right_Mishits);
        really_free(M.Mismatches_Backward);
        really_free(M.Mismatches_Forward);
        really_free(M.Two_Mismatches_At_End_Forward);
        really_free(M.Two_Mismatches_At_End);
        really_free(M.Possible_20);
        really_free(M.Possible_02);

        if(H=Zero_Mismatch(Current_Tag,L,revfmi,M,fwfmi))
        {   
...
One:
        if (H=One_Mismatch(Current_Tag,L,MAXHITS,fwfmi,revfmi,M))
        {
...
Two:
        if (H= Two_Mismatch(Current_Tag,L,MAXHITS,fwfmi,revfmi,M))
        {
...
Three:
		if (H=Three_Mismatch(Current_Tag,L,MAXHITS,fwfmi,revfmi,M))
        {
...
Four:
		 if (H= Four_Mismatch(Current_Tag,L,MAXHITS,fwfmi,revfmi,M))
        {
...
Five:
		if (H= Five_Mismatch(Current_Tag,L,MAXHITS,fwfmi,revfmi,M))
        {
 ...
 * =====================================================================================
 */
int Map_Strand(int MAX_MISMATCHES,int MAXHITS,LEN & L,BWT *fwfmi,BWT* revfmi,MEMX & M)
{
	unsigned Hits=0,H;
	char* Current_Tag=M.Current_Tag;
	M.Strand=Current_Tag[L.STRINGLENGTH];
...
unsigned Map_Strand_Guess(int MAX_MISMATCHES,int MAXHITS,LEN & L,BWT *fwfmi,BWT* revfmi,MEMX & M)
{
	...
	M.Strand=Current_Tag[L.STRINGLENGTH];
	M.Larger_Than_Ten=0;
	if(!M.Guessed && (H=Zero_Mismatch(Current_Tag,L,revfmi,M,fwfmi)))
	{
		Hits+=H;if (MAXHITS <= Hits) return Hits;
	}
	else
