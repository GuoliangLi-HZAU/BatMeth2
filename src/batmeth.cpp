...
int ORIENTATION=0; //scan all 4 possible orientation of a read by default (Cokus protocol)
int THREAD = 4;
int TRIM_LENGTH = 0;
...
inline void Print_List(bool & Header_Printed,int cntHit,int firstmismatch,int lastMismatch,std::string* tmpList,char* Header,FILE *Output_File);
void *Do_Meth( void *T);
...
LEN L;
	Analyze_File(F,L);//,L);//Get File type, read length etc..
/*	if(TRIM_LENGTH) {
...
*/	
	...
	//Load_Location(CT.LOCATIONFILE,Genome_OffsetsCT,Location_ArrayCT);//Load informatin to decode locations..
	//Load_Location(GA.LOCATIONFILE,Genome_OffsetsGA,Location_ArrayGA);
...
void *Do_Meth( void *T)
{
	...
	MLookCT.Lookupsize=MLookGA.Lookupsize=LOOKUPSIZE;
        ...
        Build_Tables(fwfmiGA,revfmiGA,MLookGA);	
*/
	std::string tmpList1[100];
	std::string tmpList2[100];
	std::string tmpList3[100];
	std::string tmpList4[100];
...
//----------- Mapping Loop  ------------------
	FILE* Output_File;
	char Out_Name[100];
	if(THREAD==1) {Output_File=File_Open(OUTPUTFILE,"w");Thread_ID=1;}
	else
	{
...
*/
		...
		for (i=0;i<=B.StringLength-1;i++){R_Rev_GA.Tag_Copy[B.StringLength-1-i]=R_Rev_CT.Tag_Copy[B.StringLength-1-i]=Char_To_C[R.Tag_Copy[i]];};//Reverse complement reads 
...
//------------- C->T scan Start ----------------------------------------------
		...
		bool bat=true;int mis=-1;
			if(Hits=Map_Strand(mis,bat,MAX_MISMATCHES,MAXHITS,L,fwfmiCT,revfmiCT,MF)){Mapped++;}
			...
			*O_CT.Buffer_End='&';*(O_CT.Buffer_End+1)='\n';
			...
			Print_List(Header_Printed,cntHit1,firstmismatch1,lastMismatch1,tmpList1,Banner,Output_File);
			...
			if(Hits=Map_Strand(mis,bat,MAX_MISMATCHES,MAXHITS,L,fwfmiGA,revfmiGA,MF_GA)){Mapped++;}
			...
			*O_RevGA.Buffer_End='&';*(O_RevGA.Buffer_End+1)='\n';
			...
			Print_List(Header_Printed,cntHit4,firstmismatch4,lastMismatch4,tmpList4,Banner,Output_File);
...
//------------- G->A scan Start ----------------------------------------------
			if(ORIENTATION==0) 
			{
				...
				*O_GA.Buffer_End='&';*(O_GA.Buffer_End+1)='\n';
				...
				Print_List(Header_Printed,cntHit2,firstmismatch2,lastMismatch2,tmpList2,Banner,Output_File);
...
//------------- C->T reversed scan Start ----------------------------------------------
				...
				*O_RevCT.Buffer_End='&';*(O_RevCT.Buffer_End+1)='\n';
				...
				Print_List(Header_Printed,cntHit3,firstmismatch3,lastMismatch3,tmpList3,Banner,Output_File);
			}
		}
...
" --orientation | -O <0|1> (All 4 possible orientations | Only 2 orientations from Original DNA Strands)\n"
