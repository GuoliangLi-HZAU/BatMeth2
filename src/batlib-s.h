...
void Print_Hits(MEMX & M,LEN & L,FILE* Output_File, char ONEFMINDEX,BWT *revfmi,BWT *fwfmi, OUTPUT & O);//moxian
...
void Search_01LX_OneSA(bool batmeth1,char* Current_Tag,struct SARange & Tag,int Count,int Start,int StringLength,int MAXHITS,BWT *fmi,LEN & L,MEMX & M);
...
int Scan(bool & batmeth,MEMX & MF,MEMX & MC,int MAX_MISMATCHES, LEN & L,BWT* fwfmi,BWT* revfmi,int Next_Mis,int & Top,int Max_Hits);
...
void Print_LocationX_bat(SARange & Tag, MEMX & M);
void Location_To_Genome_batmeth(unsigned & Location,Ann_Info & A);
bool Process_Read_batmeth(READ & R, BATREAD & B,MEMX & MF, MEMX & MC);
