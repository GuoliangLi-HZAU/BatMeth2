...
void caculateHeatmap(const char* type,int start,int end,Methy_Hash MethyList,char Strand,const char* id,FILE *methGffcg,FILE *methGffchg,FILE *methGffchh,char* chrom);
...
int main(int argc, char* argv[])
{
	...
	const char* Help_String="Command Format :   methyGff [options] -o <OUT_PREFIX> -G GENOME -gff <GFF file>/-gtf <GTF file>/-b <bed file> -m <from Split methratio outfile> [-B] [-P]\n"
		...
		"\t-nC              >= <INT> Cs per bins or genes. default:3\n"
		...
		"\t--GENE           Caculate heatmap for GENE and flank (distance) region. [Outfile: outPrefix.GENE.cg.n.txt] \n"
...
//
			...
			for(int f=FileS;f<=FileE;f++){
				...
				if(Diff)
				{
					MethGffbodyOUTFILEcg=File_Open(G_cg,"w");
					MethGffbodyOUTFILEchg=File_Open(G_chg,"w");
					MethGffbodyOUTFILEchh=File_Open(G_chh,"w");
				}
				FILE* MethGffpromoterOUTFILEcg;
				FILE* MethGffpromoterOUTFILEchg;
				FILE* MethGffpromoterOUTFILEchh;
				if(PU){
					MethGffpromoterOUTFILEcg=File_Open(P_cg,"w");
					MethGffpromoterOUTFILEchg=File_Open(P_chg,"w");
					MethGffpromoterOUTFILEchh=File_Open(P_chh,"w");
				}
				//heatmap
				//TSS
				...
				if(TSS)
				{
					MethGff_TSS_OUTFILEcg=File_Open(TSS_cg,"w");
					MethGff_TSS_OUTFILEchg=File_Open(TSS_chg,"w");
					MethGff_TSS_OUTFILEchh=File_Open(TSS_chh,"w");
				}
				//TTS
				...
				if(TTS)
				{
					MethGff_TTS_OUTFILEcg=File_Open(TTS_cg,"w");
					MethGff_TTS_OUTFILEchg=File_Open(TTS_chg,"w");
					MethGff_TTS_OUTFILEchh=File_Open(TTS_chh,"w");
				}
				//---------------------------------------------------------------------------
				...
				while(!feof(GFFINFILE))
				{
					...
					if(InputGff)
					{
						if(GTF) sscanf(Gff,"%s%s%s%u%u%s%s%s%s%s",Chrom,temp,temp,&start,&end,temp,&Strand,temp,temp,Symbol.c_str());
						else sscanf(Gff,"%s%s%s%u%u%s%s%s%s",Chrom,temp,temp,&start,&end,temp,&Strand,temp,Symbol.c_str());
						//size_t index=Symbol.find_first_of(';');
						int index=0;
						for(;Symbol[index]!=';' && Symbol[index]!='\0';index++);
						id.assign(Symbol.c_str(),0,index);
					}else if(InputBed)
					{
