...
static double to_zscore(double pval) {
  if (pval == 1)
    pval = 0.9999;
...
double stouffer_liptak(std::vector<double> &pvals,
...
void update_pval_loci(
                 const vector<PvalLocus> &pval_loci, ostream &output_encoding, double cutoff,double methdiff) {
  ...
  for(unsigned i=0;i<pval_loci.size();) {
    ...
    output_encoding << chrom << "\t" << position << "\t" << sign << "\t"
                    << context << "\t" << pval << "\t";
    if (0 <= pval && pval <= 1) {
      output_encoding << cur_locus_iter->adjust_pval << "\t" << cur_locus_iter->combined_pval << "\t"
    ...
    output_encoding << coverage_factor << "\t" << meth_factor << "\t"
                    << coverage_rest << "\t" << meth_rest << "\t" << meth_diff /*<< "\t" << state*/ <<std::endl;
...
void DistanceCorrelation::bin(const vector<PvalLocus> &loci) {
...
double DistanceCorrelation::correlation(const vector<double> &x,
  ...
  double corr= gsl_stats_correlation( gsl_x.vector.data, stride,gsl_y.vector.data, stride,x.size());
  return corr;
}


vector<double> DistanceCorrelation::correlation_table(const vector<PvalLocus> &loci) {
...
void distance_corr_matrix(BinForDistance bin_for_dist,
...
void combine_pvals(vector<PvalLocus> &loci, const BinForDistance &bin_for_distance) {
